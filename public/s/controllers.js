...
app.controller("Admin_UPDATE_GCM", function ($http, $scope, $window, $mdDialog) {
    $scope.togcm = function (i) {
        new_name = i.name;
        new_desi = i.designation;
        new_email = i.emailid;
        new_number = i.mobileno;
        new_mang = i.mngmnt;
        // new_gt = i.Gtype;
    }
    ...
    $http.get('/Admin/grievance_type').then(function (response) {
        $scope.result = response.data.info;
        $scope.newname = new_name;
        $scope.newdesignation = new_desi;
        $scope.newemailid = new_email;
        $scope.newmobileno = new_number;
        $scope.newmngmnt = new_mang;
        // $scope.newGtype = new_gt;
    });

    $scope.updategcm = function(ev) {
        $scope.form = {
            mobileno: $scope.newmobileno,
            name: $scope.newname,
            emailid: $scope.newemailid,
            designation: $scope.newdesignation,
            // Gtype: $scope.newGtype,
            mngmnt: $scope.newmngmnt,
        };

        $http.post("/Admin/UPDATEGCM", $scope.form).then(function success(response) {
            swal({
                title: "Successful",
                text: "Your changes have been updated!",
                icon: "success",
                ...
                $window.location = "/Admin/Home#!/gcm";
            });
        }, function error(response) {
            swal({
                title: "Error",

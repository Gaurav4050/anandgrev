...
var smtpTransport = nodemailer.createTransport({
  ...
  //secure: false,
  auth: {
    user: 'anandgrievanceportal@gmail.com',
    pass: 'jvcbsvowqvuceyub'
  }
});
...
router.post('/UPDATEGCM', requireLogin, function (req, res, next) {//To UPDATE selected GCM
  ...
  Member.updateuser(req.body.emailid, newvalue, function (err) {
    if (err) throw err;
  ...
  if ((req.body.mngmnt) == 1) {
    mngmnt.getUserByID(email, function (err, user) {
      if (user) {
        ...
        host = req.get('host');
        mailOptions = {
          from:'anandgrievanceportal@gmail.com',
          to: email,
    ...
    cell.getUserByID(email, function (err, user) {
      ...
      if (user) {
        ...
        console.log('cell created', password);
        var created = true;
...
router.post('/forgot_pass', function (req, res, next) {
  console.log(req.body.id,"iam hir");
  var id = req.body.id;
  admin.getUserByID(id, function (err, user) {
    if (err) throw err;
    if (!user) {
...
router.post('/termination', requireLogin, function (req, res, next) {//For Permanent Termination of either 
  var newvalues = {
    $set: {                                         //Student or Parent at end of Course Completion
      access: 'terminated'
    }
  };
...
router.post('/grvtype_add', requireLogin, function (req, res, next) { //For adding a grievance type
  ...
  Grvtype.grvtype_seq_find(grvtype, function (err, grv) {
    if (err) throw err;
    if (grv) {
